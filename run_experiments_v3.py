...
Same as run_experiments_v2.py plus two speedups (both on by default):
- KV cache during Gumbel-Max generation (--no-kv-cache to disable). The cache
  is exact up to floating-point numerics, so factual and counterfactual runs
  must use the same setting; resume refuses to mix them.
- Counterfactual runs serve calls before the intervention from the factual
  call log instead of regenerating them (--no-skip-prefix to disable). A call
  is only skipped if its agent, prompt tail and tape position match the
  factual log; on the first mismatch it falls back to normal generation.
...
def _gumbel_max_step(
    *,
    ...
    return _gumbel_max_select(
        out.logits[:, -1, :], temperature=temperature, generator=generator, top_k=top_k, top_p=top_p
    )


def _gumbel_max_select(
    logits: "torch.Tensor",
    *,
    temperature: float,
    generator: "torch.Generator",
...
class HFModelClient:
    def __init__(
        ...
        *,
        ...
        self._skip_prefix_active = False
    ...
    def begin_counterfactual(
        ...
        *,
        ...
        self._skip_prefix_active = self.skip_prefix
        self._skipped_calls = 0
    ...
    def tape_status(self) -> Dict[str, int]:
        return {
            ...
            "skipped_prefix_calls": int(self._skipped_calls),
        }
    ...
    def _generate_text_gumbel(self, prompt: str) -> str:
        ...
        if self.kv_cache:
            return self._generate_text_gumbel_cached(input_ids)
        generated: List[int] = []
    ...
    @torch.no_grad()
    def _generate_text_gumbel_cached(self, input_ids: "torch.LongTensor") -> str:
        """Same sampling as the uncached loop (one tape entry per token), but
        each step feeds only the new token and reuses past_key_values."""
        generated: List[int] = []
        out = self.model(input_ids=input_ids, use_cache=True)
        for step in range(self.max_new_tokens):
            self._tape_step()
            next_id = _gumbel_max_select(
                out.logits[:, -1, :],
                temperature=self.temperature,
                generator=self._gen,
                top_k=self.top_k,
            ...
            generated.append(next_id)
            if next_id in self._stop_ids or step == self.max_new_tokens - 1:
                break
            out = self.model(
                input_ids=torch.tensor([[next_id]], device=self.device, dtype=torch.long),
    ...
    def _try_skip_prefix_call(self, agent_name: str, prompt_preview: str) -> Optional[CallLogEntry]:
        """In counterfactual mode, return the factual entry for this call if it
        precedes the intervention and the context provably matches the factual
        run; advance the tape and generator exactly as generating it would."""
        if not (
            self._mode == "counterfactual"
            and self._skip_prefix_active
            and self._intervene_call_idx is not None
            and self._call_idx < self._intervene_call_idx
            and self._call_idx < len(self._factual_call_log)
        ):
            return None
        fe = self._factual_call_log[self._call_idx]
        if (
            fe.call_idx != self._call_idx
            or fe.agent != agent_name
            or fe.prompt_preview != prompt_preview
            or fe.tape_start != self._tape_pos
            or fe.tape_end > len(self._tape)
        ):
            print(f"[skip-prefix] call {self._call_idx} differs from factual log; generating from here on.")
            self._skip_prefix_active = False
            return None
        if fe.tape_end > fe.tape_start:
            # leave the generator where the last sampled token would have left it
            self._gen.set_state(self._tape[fe.tape_end - 1])
            vocab = self.model.get_output_embeddings().weight.shape[0]
            _sample_gumbel((vocab,), generator=self._gen, device=self.device)
        self._tape_pos = fe.tape_end
    ...
    async def create(self, messages, **kwargs) -> "CreateResult":
        ...
        skipped = self._try_skip_prefix_call(agent_name, prompt_preview)
        if skipped is not None:
            tape_start, tape_end = skipped.tape_start, skipped.tape_end
            text = skipped.response_text
        else:
...
if __name__ == "__main__":
    ...
    args.add_argument("--no-kv-cache", action="store_true",
                      help="Recompute the full sequence every token (v2 behaviour).")
    args.add_argument("--no-skip-prefix", action="store_true",
                      help="Regenerate pre-intervention calls in counterfactual runs (v2 behaviour).")
    ...
    if parsed.backend == "ollama":
        ...
        hf_client = HFModelClient(
            ...
            top_p=parsed.top_p,
            top_k=parsed.top_k,
            kv_cache=not parsed.no_kv_cache,
            skip_prefix=not parsed.no_skip_prefix,
        )
    ...
    for i in range(len(target_actions)):
        ...
        if is_resume_row and os.path.exists(factual_fp):
            ...
            if hf_client is not None and os.path.exists(tape_meta_fp):
                ...
                if bool(tape_meta.get("kv_cache", False)) != hf_client.kv_cache:
                    raise RuntimeError(
                        f"row {row_id}: factual tape was recorded with kv_cache={tape_meta.get('kv_cache', False)} "
                        f"but this run uses kv_cache={hf_client.kv_cache}; counterfactuals would not be comparable. "
                        f"Match the setting (--no-kv-cache) or rerun the factual."
                    )
            ...
            if hf_client is not None:
                ...
                tape_meta = {"tape_len": len(tape), "call_log": call_log, "kv_cache": hf_client.kv_cache}
